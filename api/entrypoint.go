...
func getAssignments(c *gin.Context) {
	...
	collector.OnHTML("div.AssignmentClass", func(e *colly.HTMLElement) {
		...
		e.ForEach("div.sg-content-grid > div.sg-asp-table-group > span.LabelCatogery > div.sg-view-quick > table.sg-asp-table", func(_ int, el *colly.HTMLElement) {
			//selection := el.DOM
			//childNodes := selection.Children().Nodes
			...
			el.ForEach("tr.sg-asp-table-data-row", func(_ int, elem *colly.HTMLElement) {
				percentages = append(percentages, strings.TrimSpace(elem.Text))
			})
		})
